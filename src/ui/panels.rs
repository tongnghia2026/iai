...
fn text_panel(ui: &mut egui::Ui, data: &UiData, actions: &mut UiActions) {
    ...
    // Hovering a row still live-previews the font; dismissing without a pick
    // restores the committed font.
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            for para in flow.document.paragraphs.iter() {
                ...
                for _inline in &para.inline_images {
                    let ordinal = img_no;
                    img_no += 1;
                    ui.horizontal(|ui| {
                        ui.add_space(6.0);
                        if ui
                            ...
                            .on_hover_text("Ảnh cùng dòng chữ — chọn để sửa kích thước")
                            .clicked()
                        {
                            actions.doc.flow_text_focus = Some(FlowTextFocus::Image(ordinal));
                        }
