...
pub struct EditorInteraction {
    ...
    /// the result to the OS clipboard. A worker polls the clipboard (a read can
    /// block while the browser renders the image) until it holds an image
    /// different from what iai wrote, or the wait times out.
    #[allow(clippy::type_complexity)]
    pub(in crate::app) pending_ext_clipboard: Option<(
        crate::app::ext_bridge::EditOrigin,
        std::sync::mpsc::Receiver<Result<crate::app::os_clipboard::OsClipboardImage, String>>,
    )>,
