...
// Drag to define rectangle; holds base mask for Add/Subtract/Intersect modes.

use super::{PointerEvent, Tool, ToolCtx, ToolResponse};
use crate::core::selection::SelectionSnapshot;

pub struct SelectionRectTool {
    pub feather: f32,
    pub anti_alias: bool,
    start_x: f32,
    start_y: f32,
    ...
    /// existed → PTS "add to selection" (free shape, not constrained square).
    freeform_add: bool,
...
impl SelectionRectTool {
    ...
    /// point so the marquee draws a perfect square.
    ...
    /// Canvas-space preview rect during drag: (x0, y0, x1, y1).
    ///
    /// Snapped to the same integer pixel grid `apply()` commits to, so the live
    /// marquee lands on the exact same pixel edges as the final marching-ants
    /// (was drawn from raw float cursor coords → up to ~1px drift on commit).
    ...
    fn apply(
        ...
        canvas.select_rect_mode(x0, y0, x1, y1, mode, self.feather, self.anti_alias);
    }
}

impl Tool for SelectionRectTool {
    ...
    fn tool_id(&self) -> crate::tools::ToolId {
        crate::tools::ToolId::SelectionRect
    }
